...
export const movieInfoAdd = async (req: Request, res: Response) => {
  ...
  // 필수 필드 검증: movie_id만 필수
  if (!movie_id) {
    console.error("필수 필드가 누락되었습니다: movie_id");
    return res.status(400).json({ message: "movie_id는 필수 항목입니다." });
  }
  ...
  try {
    ...
    // 감독 추가 (존재할 경우)
    if (Array.isArray(director_names) && director_names.length > 0) {
    ...
    // 제작사 추가 (존재할 경우)
    if (Array.isArray(production_company_names) && production_company_names.length > 0) {
